...
import com.dronesfree.security.util.SecurityUtil;
...
public class RegistrationController {
	...
	public String showRegistrationForm(ModelMap model, WebRequest request,
	...
	private User createUserAccount(RegistrationForm userAccountData,
		...
		try {
			...
			LOGGER.debug("An email address: " + userAccountData.getEmail()
					+ "exist");
	...
	private void addFieldError(String objectName, String fieldName,
		...
		LOGGER.debug("Adding field error object's:" + objectName + " field:"
				+ fieldName + " with error code:" + errorCode);
		FieldError error = new FieldError(objectName, fieldName, fieldValue,
				false, new String[] { errorCode }, new Object[] {}, errorCode);

		result.addError(error);
		LOGGER.debug("Added field error: " + error + "{} to binding result: "
				+ result);
	}
}
